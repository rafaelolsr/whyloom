...
def test_onboard_prunes_named_virtual_environments_and_dependency_caches(tmp_path):
    root = make_existing_repository(tmp_path)
    ignored_directories = (
        ".venv-deepeval-cli",
        ".venv-custom",
    ...
    for relative in ignored_directories:
        ...
        (directory / "dependency.py").write_text("def dependency():\n    return True\n", encoding="utf-8")
        (directory / "README.md").write_text("# Generated dependency\n", encoding="utf-8")
        (directory / "pyproject.toml").write_text("[project]\nname = 'generated'\n", encoding="utf-8")

    result = onboard_project(root, DEFAULT_CONFIG, history_limit=0, max_evidence=100)
    changed = result["bootstrap"]["index"]["changed"]
    manifest = json.loads((root / result["bootstrap"]["manifest"]).read_text(encoding="utf-8"))
    evidence_sources = [item["source"] for item in manifest["evidence"]]

    assert "src/service.py" in changed
    assert all(not path.startswith(ignored_directories) for path in changed)
    assert all(not path.startswith(ignored_directories) for path in evidence_sources)
